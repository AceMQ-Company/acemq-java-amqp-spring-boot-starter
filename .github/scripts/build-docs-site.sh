...
NAV='<nav class="top">
  <span class="brand"><img src="assets/acemq.png" alt="AceMQ"> Spring Boot starter</span>
...
FOOT='</main>
<footer>
  <a href="https://github.com/AceMQ-Company/acemq-java-amqp-spring-boot-starter">AceMQ Spring
  Boot starter</a> &mdash;
  ...
  RabbitMQ and Spring Boot are trademarks of Broadcom Inc. This project is not affiliated
  with it.
</footer>'
...
for f in docs/*.md; do
  ...
  pandoc "$f" \
    ...
    --metadata pagetitle="$title — AceMQ Spring Boot starter" \
    "${HIGHLIGHT[@]}" \
    --css=style.css \
...
# has written to target/site/apidocs historically and to target/reports/apidocs in
# recent versions, so both are accepted -- and a missing one is fatal rather than
# silently shipping a site whose API reference 404s.
APIDOCS=""
for candidate in \
    acemq-spring-boot-autoconfigure/target/reports/apidocs \
    acemq-spring-boot-autoconfigure/target/site/apidocs; do
  [ -d "$candidate" ] && APIDOCS="$candidate" && break
done
if [ -z "$APIDOCS" ]; then
  echo "no javadoc found; run: mvn -DskipTests javadoc:javadoc -pl acemq-spring-boot-autoconfigure" >&2
  exit 1
fi
mkdir -p "$OUT/apidocs"
